...
var c1r = 97; // outer-radius of the 5-degree tick marks (Latin)
var c1ra = 100; // outer-radius of the 5-degree tick marks (Arabic)
...
function mater(){
 ...
 // Brass Linear Gradient
  ...
  c.setAttributeNS(null, "id", "c1r_latin");
  ...
  // Lines extending beyond the Tropic of Capricorn
  var xcap_group = document.createElementNS(xmlns, "g");
  mater_group.appendChild(xcap_group);
  var xsouth = document.createElementNS(xmlns, "path");
  xcap_group.appendChild(xsouth);
  d = "M" + [0,c3r] + "L" + [0,Rcapricorn];
  xsouth.setAttributeNS(null, "d", d);
  var xnorth = document.createElementNS(xmlns, "path");
  xcap_group.appendChild(xnorth);
  d = "M" + [0,-c3r] + "L" + [0,-Rcapricorn];
  xnorth.setAttributeNS(null, "d", d);
  var xeast = document.createElementNS(xmlns, "path");
  xcap_group.appendChild(xeast);
  d = "M" + [-c3r,0] + "L" + [-Rcapricorn,0];
  xeast.setAttributeNS(null, "d", d);
  var xwest = document.createElementNS(xmlns, "path");
  xcap_group.appendChild(xwest);
  d = "M" + [c3r,0] + "L" + [Rcapricorn,0];
  xwest.setAttributeNS(null, "d", d);
  ...
  // 5 Degree Marks
  var vdegmarkings_latin = document.createElementNS(xmlns, "g");
  limb_markings_group.appendChild(vdegmarkings_latin);
  for (i=0;i<360;i+=5){
    var ptick = document.createElementNS(xmlns, "path");
    ...
    ptick.setAttributeNS(null, "d", pString);
    ptick.setAttributeNS(null, "transform", pRotateString);
    vdegmarkings_latin.appendChild(ptick);
  }

  // 5 Degree Marks (Arabic)
  var vdegmarkings_arabic = document.createElementNS(xmlns, "g");
  limb_markings_group.appendChild(vdegmarkings_arabic);
  vdegmarkings_arabic.setAttributeNS(null, "id", "vdegmarkings_arabic");
  vdegmarkings_arabic.setAttributeNS(null, "visibility", "hidden");
  var c = document.createElementNS(xmlns, "circle");
  c.cx.baseVal.value = 0;
  c.cy.baseVal.value = 0;
  c.r.baseVal.value = c1ra;
  vdegmarkings_arabic.appendChild(c);
  for (i=0;i<360;i+=5){
    var ptick = document.createElementNS(xmlns, "path");
    var pString = "M 0 "+c2r+"L 0 "+c1ra;
    var pRotateString = "rotate("+i+")";
    ptick.setAttributeNS(null, "d", pString);
    ptick.setAttributeNS(null, "transform", pRotateString);
    vdegmarkings_arabic.appendChild(ptick);
  }
 ...
 // Numerals
  ...
  for (var indx=0;indx<24;indx++){
   ...
   textGroup.setAttributeNS(null,"transform","matrix(1,0,0,1,105,105)");
   textGroup.setAttributeNS(null,"font-size","4");
   textGroup.setAttributeNS(null,"font-weight","400");
   textGroup.setAttributeNS(null,"font-optical-sizing","auto");
   textGroup.setAttributeNS(null,"font-family","Reem Kufi");
   textGroup.setAttributeNS(null, "fill", "rgb(105,85,60)");
   textGroup.setAttributeNS(null, "stroke", "none");
   textGroup.setAttributeNS(null, "text-anchor", "middle");
   textGroup.setAttributeNS(null, "id", "limbtext_arabic");
   textGroup.setAttributeNS(null, "visibility", "hidden");
   var radius = 0.505*(c1ra + c2r);
   var quad1 = ["ه","ي","يه","ك","كه","ل","له","م","مه","ن","نه","ص","صه","ع","عه","ف","فه","ض"];
   var quad2 = ["ضه","ق","قه","قي","يه","قك","كه","قل","له","قم","مه","قن","نه","قص","صه","قع","عه","قف"];
   var quad3 = ["فه","قض","ضه","ر","ره","ري","يه","رك","كه","رل","له","رم","مه","رن","نه","رص","صه","رع"];
   var quad4 = ["عه","رف","فه","رض","ضه","س","سه","سي","يه","سك","كه","سل","له","سم","مه","سن","نه","سص"];
   numerals = quad1.concat(quad2).concat(quad3).concat(quad4);
   for (var indx=0;indx<72;indx++){
     var phi = 2.5 + 5*indx;
     var char_rot = 180 + phi;
     var x = radius*Math.sin(phi*Math.PI/180);
     var y = -radius*Math.cos(phi*Math.PI/180);
     var rotationString = "rotate("+char_rot+")";
     var translationString = "translate("+x+","+y+")";
     var tx = document.createElementNS(xmlns, "text");
     tx.setAttributeNS(null,"x",0);
     tx.setAttributeNS(null,"y",0);
  ...
  var prayerline_group = document.getElementById("prayerline_group");
  prayerline_group.setAttribute("stroke-width", .7);
  prayerline_group.setAttribute("stroke-dasharray", "1,.5");
  prayerline_group.setAttribute("clip-path", "url(#CapricornClip)");
  var p = document.createElementNS(xmlns, "path");
  p.setAttributeNS(null, "id", "fajr");
  prayerline_group.appendChild(p);
  var p = document.createElementNS(xmlns, "path");
  p.setAttributeNS(null, "id", "shafaq");
  prayerline_group.appendChild(p);
  var c = document.createElementNS(xmlns, "circle");
  c.setAttributeNS(null, "id", "twilight_circle");
  c.setAttribute(null, "cx", 0);
  prayerline_group.appendChild(c);
  var p = document.createElementNS(xmlns, "path");
  p.setAttributeNS(null, "id", "asr");
  prayerline_group.appendChild(p);
  var p = document.createElementNS(xmlns, "path");
  p.setAttributeNS(null, "id", "zuhr");
  prayerline_group.appendChild(p);
...
// Zenith Circle (Altitude = 85)
  ...
  // East Sky Compliment (Below-Horizon)
  d = "M"+[0,y2]+"L"+[0,-Rcapricorn];
  if (latitude > 90-obliquity){
    d += "A"+[Rcapricorn,Rcapricorn,0,0,0,0,Rcapricorn]+"L"+[0,y1];
    d += "A"+[r_altitude,r_altitude,0,1,1,0,y2]+"Z";
  }
  else {
    d += "A"+[Rcapricorn,Rcapricorn,0,0,0,-xAC,yAC];
    d +="A"+[r_altitude,r_altitude,0,0,1,0,y2]+"Z";
  }
  if (latitude==0){
    d = "M"+[0,0]+"L"+[0,-Rcapricorn]+"A"+[Rcapricorn,Rcapricorn,0,0,0,-Rcapricorn,0]+"L"+[0,0]+"Z";
  }
  var p = document.getElementById("belowHorizonClipPath");
  p.setAttribute("d", d);
...
//-----------------------------------
function circle_from_3points(x1,y1,x2,y2,x3,y3){
  var a = x2-x1;
  var b = y2-y1;
  var c = (Math.pow(x2,2) - Math.pow(x1,2) + Math.pow(y2,2) - Math.pow(y1,2))/2;
  var d = x3-x1;
  var e = y3-y1;
  var f = (Math.pow(x3,2) - Math.pow(x1,2) + Math.pow(y3,2) - Math.pow(y1,2))/2;
  var x_center = ((c/b) - (f/e)) / ((a/b) - (d/e));
  var y_center = (c - a*x_center) / b;
  var r = Math.sqrt(Math.pow(x_center - x1,2) + Math.pow(y_center - y1,2));
...
//--- Islamic Prayer Lines ----------
//-----------------------------------
function draw_prayerlines(latitude){
  var latitude=Number(latitude);

  // Paths
  var p_fajr = document.getElementById("fajr");
  var p_shafaq = document.getElementById("shafaq");
  var c_twilight = document.getElementById("twilight_circle");
  var p_asr = document.getElementById("asr");
  var p_zuhr = document.getElementById("zuhr");
  ...
  yAC = (Math.pow(Rcapricorn,2) - Math.pow(r_altitude,2) + Math.pow(yca,2))/(2*yca); // y-intersection of altitude arc and Tropic of Capricorn
  xAC = Math.sqrt(Math.pow(Rcapricorn,2) - Math.pow(yAC,2)); // x-intersection of altitude arc and Tropic of Capricorn
  yACn = (Math.pow(Rcancer,2) - Math.pow(r_altitude,2) + Math.pow(yca,2))/(2*yca); // y-intersection of altitude arc and Tropic of Cancer
  xACn = Math.sqrt(Math.abs(Math.pow(Rcancer,2) - Math.pow(yACn,2))); // x-intersection of altitude arc and Tropic of Cancer

  if (Math.abs(y2)<Rcancer){
    c_twilight.setAttribute("r", "none");
    if (latitude+altitude!=0){
      m = [-xAC,yAC];
      a = [r_altitude,r_altitude,0,0,1,-xACn,yACn];
      d = "M"+m+"A"+a;
      p_fajr.setAttribute("d", d);

      m = [xAC,yAC];
      a = [r_altitude,r_altitude,0,0,0,xACn,yACn];
      d = "M"+m+"A"+a;
      p_shafaq.setAttribute("d", d);
    }
    else{
      var theta_cap = Math.asin(-y2/Rcapricorn);
      xAC = Rcapricorn * Math.cos(theta_cap);
      var theta_can = Math.asin(-y2/Rcancer);
      xACn = Rcancer * Math.cos(theta_can);
      m1 = [-xAC,y2];
      m2 = [-xACn,y2];
      d = "M"+m1+"L"+m2;
      p_fajr.setAttribute("d", d);
    ...
    p_fajr.setAttribute("d", "");
    p_shafaq.setAttribute("d", "");
    c_twilight.setAttribute("cy", yca);
    c_twilight.setAttribute("r", r_altitude);
  }

  // Noon shadows
  var capel = 90-obliquity-latitude; 
  var eqel = 90-latitude;
  var canel = 90-Math.sign(latitude-obliquity)*(latitude-obliquity);
  var cap_shadow =  1 / Math.tan(capel * Math.PI/180);
  var eq_shadow =  1 / Math.tan(eqel * Math.PI/180);
  var can_shadow =  1 / Math.tan(canel * Math.PI/180);

  // Asr angles
  var asr_capel = (180/Math.PI) * Math.atan2(1,1+cap_shadow);
  var asr_eqel = (180/Math.PI) * Math.atan2(1,1+eq_shadow);
  var asr_canel = (180/Math.PI) * Math.atan2(1,1+can_shadow);
  
  // Asr x1,y1: Summer Solstice
  altitude = asr_canel;
  y1 = Req*Math.tan((Math.PI/180)*(180-altitude-latitude)/2);
  y2 = Req*Math.tan((Math.PI/180)*(altitude-latitude)/2);
  r_altitude = (y1-y2)/2;
  ...
  yACn = (Math.pow(Rcancer,2) - Math.pow(r_altitude,2) + Math.pow(yca,2))/(2*yca); // y-intersection of altitude arc and Tropic of Cancer
  xACn = Math.sqrt(Math.abs(Math.pow(Rcancer,2) - Math.pow(yACn,2))); // x-intersection of altitude arc and Tropic of Cancer
  var asr_theta1 = Math.atan2(yACn,xACn);
  var asr_x1 = -Rcapricorn * Math.cos(asr_theta1);
  var asr_y1 = -Rcapricorn * Math.sin(asr_theta1);
  ...
  r_altitude = (y1-y2)/2;
  yca = y1-r_altitude;
  yAq = (Math.pow(Req,2) - Math.pow(r_altitude,2) + Math.pow(yca,2))/(2*yca); // y-intersection of altitude arc and Equator
  xAq = Math.sqrt(Math.abs(Math.pow(Req,2) - Math.pow(yAq,2))); // x-intersection of altitude arc and Equator
  var asr_theta2 = Math.atan2(yAq,xAq);
  var asr_x2 = -Req * Math.cos(asr_theta2);
  var asr_y2 = -Req * Math.sin(asr_theta2);

  // Asr x3,y3: Winter Solstice
  altitude = asr_capel;
  y1 = Req*Math.tan((Math.PI/180)*(180-altitude-latitude)/2);
  y2 = Req*Math.tan((Math.PI/180)*(altitude-latitude)/2);
  r_altitude = (y1-y2)/2;
  yca = y1-r_altitude;
  yAC = (Math.pow(Rcapricorn,2) - Math.pow(r_altitude,2) + Math.pow(yca,2))/(2*yca); // y-intersection of altitude arc and Tropic of Cancer
  xAC = Math.sqrt(Math.abs(Math.pow(Rcapricorn,2) - Math.pow(yAC,2))); // x-intersection of altitude arc and Tropic of Cancer
  var asr_theta3 = Math.atan2(yAC,xAC);
  var asr_x3 = Math.sign(capel) * (-Rcancer * Math.cos(asr_theta3));
  var asr_y3 = -Rcancer * Math.sin(asr_theta3);

  // Asr arc
  [x_asr, y_asr, r_asr] = circle_from_3points(asr_x1,asr_y1,asr_x2,asr_y2,asr_x3,asr_y3);
  if ((asr_y3-asr_y1)*(asr_x2-asr_x1) >= (asr_y2-asr_y1)*(asr_x3-asr_x1)){ 
    var asr_sweep_flag = 1;
  }
  else{
    var asr_sweep_flag = 0;
  } 
  if ((latitude > 90-obliquity) && ((asr_y3-asr_y1)*(x_asr-asr_x1) >= (y_asr-asr_y1)*(asr_x3-asr_x1))){ 
    var asr_large_arc_flag = 1;
  }
  else{
    var asr_large_arc_flag = 0;
  }  
  m = [asr_x1,asr_y1];
  m2 = [asr_x3,asr_y3];
  a = [r_asr,r_asr,0,asr_large_arc_flag,asr_sweep_flag,asr_x3,asr_y3];
  if (r_asr < 10*Rlimb){ 
    d = "M"+m+"A"+a;
  }
  else{
  ...
  p_asr.setAttribute("d", d);
  p_asr.setAttribute("clip-path", "url(#belowHorizonClip)");

  // Zuhr angles
  var zuhr_capel = (180/Math.PI) * Math.atan2(1, 0.25 + cap_shadow);
  var zuhr_eqel = (180/Math.PI) * Math.atan2(1, 0.25 + eq_shadow);
  var zuhr_canel = (180/Math.PI) * Math.atan2(1, 0.25 + can_shadow);

  // Zuhr x1,y1: Summer Solstice
  altitude = zuhr_canel;
  y1 = Req*Math.tan((Math.PI/180)*(180-altitude-latitude)/2);
  y2 = Req*Math.tan((Math.PI/180)*(altitude-latitude)/2);
  r_altitude = (y1-y2)/2;
  ...
  yACn = (Math.pow(Rcancer,2) - Math.pow(r_altitude,2) + Math.pow(yca,2))/(2*yca); // y-intersection of altitude arc and Tropic of Cancer
  xACn = Math.sqrt(Math.abs(Math.pow(Rcancer,2) - Math.pow(yACn,2))); // x-intersection of altitude arc and Tropic of Cancer
  var zuhr_theta1 = Math.atan2(yACn,xACn);
  var zuhr_x1 = -Rcapricorn * Math.cos(zuhr_theta1);
  var zuhr_y1 = -Rcapricorn * Math.sin(zuhr_theta1);
  ...
  yAq = (Math.pow(Req,2) - Math.pow(r_altitude,2) + Math.pow(yca,2))/(2*yca); // y-intersection of altitude arc and Equator
  xAq = Math.sqrt(Math.abs(Math.pow(Req,2) - Math.pow(yAq,2))); // x-intersection of altitude arc and Equator
  var zuhr_theta2 = Math.atan2(yAq,xAq);
  var zuhr_x2 = -Req * Math.cos(zuhr_theta2);
  var zuhr_y2 = -Req * Math.sin(zuhr_theta2);

  // Zuhr x3,y3: Winter Solstice
  altitude = zuhr_capel;
  y1 = Req*Math.tan((Math.PI/180)*(180-altitude-latitude)/2);
  y2 = Req*Math.tan((Math.PI/180)*(altitude-latitude)/2);
  r_altitude = (y1-y2)/2;
  ...
  yAC = (Math.pow(Rcapricorn,2) - Math.pow(r_altitude,2) + Math.pow(yca,2))/(2*yca); // y-intersection of altitude arc and Tropic of Cancer
  xAC = Math.sqrt(Math.abs(Math.pow(Rcapricorn,2) - Math.pow(yAC,2))); // x-intersection of altitude arc and Tropic of Cancer
  var zuhr_theta3 = Math.atan2(yAC,xAC);
  var zuhr_x3 = Math.sign(capel) * (-Rcancer * Math.cos(zuhr_theta3));
  var zuhr_y3 = -Rcancer * Math.sin(zuhr_theta3);

  // Zuhr arc
  [x_zuhr, y_zuhr, r_zuhr] = circle_from_3points(zuhr_x1,zuhr_y1,zuhr_x2,zuhr_y2,zuhr_x3,zuhr_y3);
  if ((zuhr_y3-zuhr_y1)*(zuhr_x2-zuhr_x1) >= (zuhr_y2-zuhr_y1)*(zuhr_x3-zuhr_x1)){ 
    var zuhr_sweep_flag = 1;
  }
  else{
    var zuhr_sweep_flag = 0;
  } 
  if ((latitude > 90-obliquity) && ((zuhr_y3-zuhr_y1)*(x_zuhr-zuhr_x1) >= (y_zuhr-zuhr_y1)*(zuhr_x3-zuhr_x1))){ 
    var zuhr_large_arc_flag = 1;
  }
  else{
    var zuhr_large_arc_flag = 0;
  }  
  m = [zuhr_x1,zuhr_y1];
  m2 = [zuhr_x3,zuhr_y3];
  a = [r_zuhr,r_zuhr,0,zuhr_large_arc_flag,zuhr_sweep_flag,zuhr_x3,zuhr_y3];
  if (r_zuhr < 10*Rlimb){ 
    d = "M"+m+"A"+a;
  }
  else{
  ...
  p_zuhr.setAttribute("d", d);
  p_zuhr.setAttribute("clip-path", "url(#belowHorizonClip)");
