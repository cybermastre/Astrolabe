...
function nav_button_init(){
	...
	var tn = document.createTextNode("ASTROLABE");
	tx.appendChild(tn);
	g.appendChild(tx);
	bannertop.appendChild(g);

	var button_names = ["navbutton1", "navbutton2", "navbutton3", "navbutton4"];
	var button_labels = [["What?"], ["Pieces"], ["Links"], ["Home","Page"]];
	...
	$("#presmode_div").on("vmouseover", function(){
		$(this).css("cursor","pointer");
		});
	document.getElementById("presmode_div").addEventListener("click", presmode_pressed);
...
function navbutton4_pressed(){window.location='https://www.alexboxer.com';}

function presmode_pressed(){
	$("#header").toggle(500);
	var prestext = $("#presmode_div > p").text();
	if (prestext == "◥"){
		$("#presmode_div > p").text("◣");
		$("#scroller").css({
			"max-height":"24em",
